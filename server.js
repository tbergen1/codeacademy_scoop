// dependencies for file read/writes
const fs = require('fs');
...
// saves a comment on an article
function saveComment(url, req) {
  ...
  // add new comment to existing database comment registry
  Object.assign(database.comments, newComment);
...
function updateComment(url, req) {
  // create response object
  const res = { status: null, body: {} };
...
// upvotes a comment on an article
function upvoteComment(url, req) {
  // create response object
  const res = { status: null, body: {} };
...
// downvotes comment on an article
function downvoteComment(url, req) {
  // create response object
  const res = { status: null, body: {} };
...
// loads database into JSON format from YAML file
function loadDatabase() {
  const database = fs.readFileSync('database.yml');

  return YAML.parse(database);
}

// saves database into YAML file
function saveDatabase(database) {
  const yamlDatabase = YAML.stringify(database, 4);

  return fs.writeFileSync('database.yml', yamlDatabase);
}
